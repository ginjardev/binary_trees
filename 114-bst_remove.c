...
bst_t *bst_delete(bst_t *root, bst_t *node)
{
	bst_t *parent = node->parent, *inheritor = NULL;
	...
	/* Case 3: Two children */
	inheritor = inorder_successor(node->right);
	node->n = inheritor->n;

	return (bst_delete(root, inheritor));
}
 ...
 *              is replaced with its first in-order inheritor.
 */
bst_t *bst_remove(bst_t *root, int value)
{
